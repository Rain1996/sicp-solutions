...
(cond ((self-evaluating? exp) exp)
      ...
      ((begin? exp)
            (eval-sequence (begin-actions exp) env))
      ((cond? exp) (eval (cond->if exp) env))
      ((and? exp) (eval (and->if exp) env))       ; special forms and
      ((or? exp) (eval (or->if exp) env))         ; special forms or
      ((application? exp)
            (apply (eval (operator exp) env)
                   (list-of-values (operands exp) env)))
      (else (error "Unknown expression type -- EVAL" exp))))
...
(define (and->if exp)
(and->if-helper (and-clauses exp)))

(define (and->if-helper clauses)
    (if (null? clauses)
        'true
        (let ((first (and-first-clause clauses))
              (rest (and-rest-clause clauses)))
            (make-if first (cons 'and rest) 'false))))
...
(if (null? clauses)
    ...
    (let ((first (or-first-clause clauses))
          (rest (or-rest-clause clauses)))
        (make-if first 'ture (cons 'or  rest)))))
